...
contract Residents is IResidents, State {
    ...
    function verifyResident(address addr, uint residentId) external onlyOwner {
        require(!_isResident(addr), "address already associated to an eResident");
        require(_residentToAddress[residentId] == address(0), "resident already associated to an address");
        _addressToResident[addr] = residentId;
        _residentToAddress[residentId] = addr;
    }
}